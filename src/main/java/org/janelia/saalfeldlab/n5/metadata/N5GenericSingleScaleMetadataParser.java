...
public class N5GenericSingleScaleMetadataParser implements N5MetadataParser<N5SingleScaleMetadata>, N5MetadataWriter<N5SingleScaleMetadata> {
  ...
  public Optional<N5SingleScaleMetadata> parseMetadata(N5Reader n5, N5TreeNode node) {
	...
	try {
	  ...
	  final double max = Optional.ofNullable(n5.getAttribute(node.getPath(), MAX, double.class)).orElseGet(() -> PainteraBaseMetadata.maxForDataType(attributes.getDataType()));
	  //TODO how to use this?
	  final Boolean isLabelMultiset = Optional.ofNullable(n5.getAttribute(node.getPath(), IS_LABEL_MULTISET, Boolean.class)).orElse(false);
	  ...
	  N5SingleScaleMetadata metadata = new N5SingleScaleMetadata(path, transform, downsamplingFactors, resolution, offset, "pixel", attributes, min, max);
